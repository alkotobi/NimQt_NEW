...
proc sqlInsert(self:User):string=
  result = "INSERT INTO user (name,login,pass) VALUES (?,?,?);"
proc sqlUpdate(self:User,id:int):string=
  result = &"UPDATE user set name = ? , login = ? , pass = ? WHERE id ={id}"
...
db.exec(sqlCreateTable(user).sql())
